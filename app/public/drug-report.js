...
(() => {
  ...
  card.innerHTML = `<details id="drugMonthDetails">
    ...
    <div class="cost-note" id="drugMonthCostNote" role="note" hidden></div>
    ...
    <p class="muted">ส่วนต่าง = ยอดหลังส่วนลด − ทุนที่บันทึกตอนออกใบ ไม่ใช่ต้นทุนแยกล็อต และยังไม่หักค่าเช่า ค่าแรง หรือยาสูญเสีย
      · รายการที่มีแก้บิล/คืนเงินจะแสดงว่า “ต้องตรวจ” เพื่อให้ตรวจใบเสร็จที่เกี่ยวข้อง</p>
  ...
  function render() {
    ...
    const rows = [...data.rows].sort((a, b) => sort.value === 'name' ? a.name.localeCompare(b.name, 'th')
      ...
      : b.qty - a.qty);
    showCostNote('drugMonthCostNote', {
      unknown_cost_lines: rows.reduce((sum, r) => sum + (r.unknown_cost_lines || 0), 0),
      unknown_cost_items: rows.filter(r => r.unknown_cost_lines).map(r => ({ line_type: 'drug', name: r.name, lines: r.unknown_cost_lines })),
    }, { drugOnly: true });
    table.innerHTML = `<thead><tr><th>ยา / หน่วย</th><th class="num">จำนวนตามบิล</th><th class="num">ก่อนลด</th><th class="num">ส่วนลดแบ่ง</th><th class="num">หลังลด</th><th class="num">ทุนตามบิล</th><th class="num">ส่วนต่าง</th><th class="num">คงเหลือตอนนี้</th><th>วันหมดอายุที่ต้องตรวจ</th></tr></thead><tbody>` +
      rows.map(r => `<tr><td>${esc(r.name)}<br><span class="muted">${esc(r.unit)}${r.drug_id != null ? ` · #${r.drug_id}` : ' · รายการเก่า'}</span>
        ...
        <td class="num">${amount(r.net)}</td><td class="num">${r.unknown_cost_lines ? ReportCost.amount(r.cost, true) : amount(r.cost)}</td><td class="num">${r.unknown_cost_lines ? ReportCost.amount(r.profit, true) : amount(r.profit)}</td>
  ...
  async function loadDrugMonth() {
    ...
    showCostNote('drugMonthCostNote', { unknown_cost_lines: 0 });
