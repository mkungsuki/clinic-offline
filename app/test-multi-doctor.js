...
try{
 ...
 for(const from of [13,14])test('populated '+from+' → 15 migration preserves records and leaves both new columns NULL',()=>{
  ...
  copy.exec("DROP TABLE audit_changes; DELETE FROM settings WHERE key='audit_changes_since'; ALTER TABLE drugs DROP COLUMN default_dose_json; ALTER TABLE services DROP COLUMN cost; ALTER TABLE appointments DROP COLUMN doctor_id; ALTER TABLE visits DROP COLUMN preferred_doctor_id;"+(from===13?'DROP TABLE appointment_events;':'')+'PRAGMA user_version='+from);
